...
const cf::TypeSys::TypeInfoT* EntWeaponCrossbowT::GetType() const
{
    return &TypeInfo;
 // return &EntWeaponCrossbowT::TypeInfo;
}

void* EntWeaponCrossbowT::CreateInstance(const cf::TypeSys::CreateParamsT& Params)
{
    return new EntWeaponCrossbowT(*static_cast<const EntityCreateParamsT*>(&Params));
}

const cf::TypeSys::TypeInfoT EntWeaponCrossbowT::TypeInfo(GetBaseEntTIM(), "EntWeaponCrossbowT", "EntWeaponT", EntWeaponCrossbowT::CreateInstance, NULL /*MethodsList*/);
...
void EntWeaponCrossbowT::NotifyTouchedBy(BaseEntityT* Entity)
{
    ...
    if (!cf::GameSys::GameImplT::GetInstance().GetCarriedWeapon(WEAPON_SLOT_CROSSBOW)->ServerSide_PickedUpByEntity(dynamic_cast<EntHumanPlayerT*>(Entity))) return;
