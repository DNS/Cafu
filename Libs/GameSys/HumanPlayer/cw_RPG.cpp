        ...
        case 1: // Fidget1 (rocket inserted)
            if (PlayerCommand.Keys & (PCK_Fire1 | PCK_Fire2))
            {
                ...
                if (ThinkingOnServerSide)
                {
                    ...
                    const VectorT NewEntOrigin(HumanPlayer->GetCameraOriginWS() - VectorT(0.0, 0.0, 8.0)+scale(ViewDir, 41.0)+scale(HumanPlayer->GetPlayerVelocity(), double(PlayerCommand.FrameTime)));
