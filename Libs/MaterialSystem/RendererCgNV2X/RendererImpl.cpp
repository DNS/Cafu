...
bool RendererImplT::IsSupported() const
{
    ...
    // Check if we can get a proper CgContext, and if the NV2X fragment and vertex shader profiles are supported.
    CGcontext TestContext=cgCreateContext();
    if (TestContext==NULL) return false;

    const bool ProfilesNV2XSupported=cgGLIsProfileSupported(CG_PROFILE_VP20)==CG_TRUE && cgGLIsProfileSupported(CG_PROFILE_FP20)==CG_TRUE;

    // Make sure that the context is destroyed again, such that subsequent calls to this function of other renderers find a clean environment.
    cgDestroyContext(TestContext);
    TestContext=NULL;

    if (!ProfilesNV2XSupported) return false;
...
{
    return "Renderer for NVidia NV2X (and higher) GPUs, based on Cg.";
}
...
void RendererImplT::RenderMesh(const MeshT& Mesh)
{
    ...
    /* // Render the texture space basis vectors (axes).
    if (CurrentRenderAction==LIGHTING)
    {
        ShaderT* SolidShader=NULL;

        // Find the "A_Solid" shader.
        for (unsigned long ShaderNr=0; ShaderNr<GetShaderRepository().Size(); ShaderNr++)
            if (GetShaderRepository()[ShaderNr]->GetName()=="A_Solid")
            {
                SolidShader=GetShaderRepository()[ShaderNr];
                break;
            }
        ...
        if (CurrentShader!=NULL) CurrentShader->Deactivate();
        SolidShader->Activate();

        for (unsigned long VertexNr=0; VertexNr<Mesh.Vertices.Size(); VertexNr++)
        {
            const MeshT::VertexT& V=Mesh.Vertices[VertexNr];

            MeshT M(MeshT::Lines);
            M.Vertices.PushBackEmpty(6);

            const float scale=100.0;

            M.Vertices[0].SetOrigin(V.Origin[0],                     V.Origin[1],                     V.Origin[2]                    ); M.Vertices[0].SetColor(1.0f, 0.0f, 0.0f);
            M.Vertices[1].SetOrigin(V.Origin[0]+V.Normal  [0]*scale, V.Origin[1]+V.Normal  [1]*scale, V.Origin[2]+V.Normal  [2]*scale); M.Vertices[1].SetColor(1.0f, 0.0f, 0.0f);
            M.Vertices[2].SetOrigin(V.Origin[0],                     V.Origin[1],                     V.Origin[2]                    ); M.Vertices[2].SetColor(0.0f, 1.0f, 0.0f);
            M.Vertices[3].SetOrigin(V.Origin[0]+V.Tangent [0]*scale, V.Origin[1]+V.Tangent [1]*scale, V.Origin[2]+V.Tangent [2]*scale); M.Vertices[3].SetColor(0.0f, 1.0f, 0.0f);
            M.Vertices[4].SetOrigin(V.Origin[0],                     V.Origin[1],                     V.Origin[2]                    ); M.Vertices[4].SetColor(0.0f, 0.0f, 1.0f);
            M.Vertices[5].SetOrigin(V.Origin[0]+V.BiNormal[0]*scale, V.Origin[1]+V.BiNormal[1]*scale, V.Origin[2]+V.BiNormal[2]*scale); M.Vertices[5].SetColor(0.0f, 0.0f, 1.0f);

            SolidShader->RenderMesh(M);
        }

        SolidShader->Deactivate();
        if (CurrentShader!=NULL) CurrentShader->Activate();
    } */
}
...
CGcontext RendererImplT::GetCgContext() const
{
    return CgContext;
}
