...
class Shader_A_Diff_Light_Norm_Luma : public ShaderT
{
    ...
    void Initialize()
    {
        ...
        FragmentShader=UploadCgProgram(RendererImplT::GetInstance().GetCgContext(), CG_PROFILE_ARBFP1,
            ...
            "      uniform sampler2D LuminanceMapSampler : TEXUNIT5)    \n"
            " {                                                         \n"
            "     const float4 DiffuseC    =     tex2D(DiffuseMapSampler,   InTexCoord_Diff);               \n"
            "     const float4 SpecularC   =     tex2D(SpecMapSampler,      InTexCoord_Diff);               \n"
            "     const float4 LuminanceC  =     tex2D(LuminanceMapSampler, InTexCoord_Diff);               \n"
            "     const float4 LightMapC   =     tex2D(LightMapSampler,     InTexCoord_LiMa);               \n"
            "     const float4 LightMapDirC=     tex2D(LightDirMapSampler,  InTexCoord_LiMa);               \n"
            "     const float3 LightMapDir =2.0*(LightMapDirC.xyz-0.5);                                     \n"
            "     const float3 Normal      =2.0*(tex2D(NormalMapSampler,    InTexCoord_Diff).xyz-0.5);      \n"
            "                                                                                               \n"
            "     const float3 HalfwayDir  =normalize(normalize(EyeDir)+LightMapDir);                       \n"
            "                                                                                               \n"
            "     const float4 diff        =float4(saturate(dot(LightMapDir, Normal)).xxx, 1.0);            \n"
            "     const float4 spec        =float4(pow(saturate(dot(HalfwayDir, Normal)), 32.0).xxx, 0.0);  \n"
            "                                                                                               \n"
            "     const float4 LightMapCorrectedC=float4(LightMapC.xyz/LightMapDirC.w, 1.0);                \n"
            "                                                                                               \n"
            "     // Hmmm. There are certainly more ways to combine all these numbers -                     \n"
            "     // this has been an arbitrary choice.                                                     \n"
            "     OutColor=LightMapCorrectedC*(diff*DiffuseC + spec*SpecularC) + LuminanceC*InColor;        \n"
            " }                                                                                             \n");
    }
