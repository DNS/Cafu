...
class Shader_A_Diff_Norm_Luma : public ShaderT
{
    private:
    ...
    void Initialize()
    {
        ...
        FragmentShader=UploadCgProgram(RendererImplT::GetInstance().GetCgContext(), CG_PROFILE_ARBFP1,
            ...
            " {                                                                         \n"
            "     float4 DiffuseC  =     tex2D(DiffuseMapSampler  , InTexCoord);        \n"
            "     float4 NormFac   =2.0*(tex2D(NormalMapSampler   , InTexCoord).z-0.5); \n"
            "     float4 LuminanceC=     tex2D(LuminanceMapSampler, InTexCoord);        \n"
            "                                                                           \n"
            "     NormFac.a   =1.0;                                                     \n"
            "     LuminanceC.a=0.0;                                                     \n"
            "                                                                           \n"
            "     // Hmmm. Choose the uncommented line as the 'right' one...            \n"
            "  // OutColor=(DiffuseC*NormFac + LuminanceC)*InColor;                     \n"
            "  // OutColor=DiffuseC*NormFac*InColor + LuminanceC;                       \n"
            "     OutColor=DiffuseC*NormFac + LuminanceC*InColor;                       \n"
            " }                                                                         \n");
    }
    ...
    {
        static const std::string Name="A_Diff_Norm_Luma";
...
static Shader_A_Diff_Norm_Luma MyShader;
