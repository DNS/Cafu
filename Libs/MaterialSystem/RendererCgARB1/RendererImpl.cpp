...
bool RendererImplT::IsSupported() const
{
    ...
    // Check if we can get a proper CgContext, and if the ARBv1 fragment and vertex shader profiles are supported.
    CGcontext TestContext=cgCreateContext();
    if (TestContext==NULL) return false;

    const bool ProfilesARB1Supported=cgGLIsProfileSupported(CG_PROFILE_ARBVP1)==CG_TRUE && cgGLIsProfileSupported(CG_PROFILE_ARBFP1)==CG_TRUE;
    ...
    if (!ProfilesARB1Supported) return false;
...
{
    return "Renderer for ARB vertex and fragment programs version 1.";
}
