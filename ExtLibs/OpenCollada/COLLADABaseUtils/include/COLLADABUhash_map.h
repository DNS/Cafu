...
#if defined (COLLADABU_HAVE_STD_UNORDERED_MAP)
    using namespace std;
 
	template<class X, class Y > 
	class hash_map : public std::unordered_map<X,Y>
	{ public:
	hash_map(){}
	hash_map(const hash_map&a):std::unordered_map<X,Y>(a){}
	hash_map&operator=(const hash_map&a){std::unordered_map<X,Y>::operator=(*this,a);return this;}
	};

	template<class X> 
	class hash_set : public std::unordered_set<X>
	{ public:
	hash_set(){}
	hash_set(const hash_set&a):std::unordered_set<X>(a){}
	hash_set&operator=(const hash_set&a){std::unordered_set<X>::operator=(*this,a);return this;}
	};
#elif defined (COLLADABU_HAVE_TR1_UNORDERED_MAP)
